...
abstract class ExteraExtension {
    @get:Nested abstract val telegram: TelegramExtension

    @get:Nested abstract val shadow: ShadowExtension

    @get:Nested abstract val r8: R8Extension

    @get:Nested abstract val bundle: BundleExtension
    ...
    fun telegram(action: Action<in TelegramExtension>) = action.execute(telegram)

    fun shadow(action: Action<in ShadowExtension>) = action.execute(shadow)

    fun r8(action: Action<in R8Extension>) = action.execute(r8)
