...
abstract class D8WorkAction : WorkAction<D8Parameters> {
    ...
    override fun execute() {
        ...
        val work = parameters.workDir.get().asFile
            .apply { deleteRecursively(); mkdirs() }

        val command = D8Command.builder(GradleDiagnosticsHandler())
    ...
    private fun pack(input: File, work: File) {
        ...
        ZipOutputStream(output.outputStream().buffered()).use { out ->
            ...
            for (dex in dexFiles)
                out.writeEntry(dex.name, dex.readBytes())

            ZipFile(input).use { zip ->
                zip.entries()
                    .asSequence()
                    .filter(ZipEntry::isResource)
                    .forEach { entry ->
